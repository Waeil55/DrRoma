          ...
          {!activeDocId && currentView === 'settings' && (
            <div className="flex-1 overflow-y-auto p-10 lg:p-16 custom-scrollbar bg-gray-50 dark:bg-[#0a0a0c]">
              <div className="max-w-3xl mx-auto">
                 <h1 className="text-4xl font-black text-gray-800 dark:text-white tracking-tighter mb-8 flex items-center gap-4"><Settings className="text-[var(--accent-color)]"/> System Settings</h1>
...
function SidebarBtn({ icon: Icon, label, active, onClick, highlight }) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center gap-1.5 transition-all group w-full`}
    >
      <div className={`w-14 h-14 rounded-[1.25rem] flex items-center justify-center transition-all ${
        active
          ? highlight ? 'bg-[var(--accent-color)] text-white shadow-xl shadow-[var(--accent-color)]/40 scale-105' : 'bg-gray-200 dark:bg-zinc-800 text-gray-800 dark:text-white shadow-lg scale-105'
      ...
      <span className={`text-[9px] font-bold uppercase tracking-widest transition-all ${active ? (highlight ? 'text-[var(--accent-color)]' : 'text-gray-800 dark:text-white') : 'text-gray-400 dark:text-zinc-600 group-hover:text-gray-600 dark:group-hover:text-zinc-400'}`}>
        {label}
      </span>
    </button>
  );
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 gap-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-zinc-400 text-lg max-w-2xl leading-relaxed">Your secure, local medical knowledge base. Upload materials and let the elite AI extract exams, notes, and cards seamlessly.</p>
          </div>
          <label className={`cursor-pointer bg-white dark:bg-zinc-100 text-zinc-950 hover:bg-gray-100 dark:hover:bg-zinc-200 px-8 py-5 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center gap-4 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1 ${isUploading ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div onClick={() => setView('notes')} className="cursor-pointer hover:-translate-y-1 transition-all bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md border border-gray-200 dark:border-zinc-800/60 p-8 rounded-[2rem] flex items-center gap-6 shadow-lg hover:shadow-xl">
            ...
            <div><p className="text-4xl font-black text-gray-800 dark:text-white">{notes.length}</p><p className="text-xs font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mt-2">Generated Notes</p></div>
          </div>
          <div onClick={() => setView('flashcards')} className="cursor-pointer hover:-translate-y-1 transition-all bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md border border-gray-200 dark:border-zinc-800/60 p-8 rounded-[2rem] flex items-center gap-6 shadow-lg hover:shadow-xl">
            <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center shrink-0"><Layers size={32}/></div>
            <div><p className="text-4xl font-black text-gray-800 dark:text-white">{totalCardsCount}</p><p className="text-xs font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mt-2">Active Flashcards</p></div>
          </div>
          <div onClick={() => setView('exams')} className="cursor-pointer hover:-translate-y-1 transition-all bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md border border-gray-200 dark:border-zinc-800/60 p-8 rounded-[2rem] flex items-center gap-6 shadow-lg hover:shadow-xl">
            <div className="w-16 h-16 rounded-2xl bg-purple-500/10 text-purple-500 flex items-center justify-center shrink-0"><GraduationCap size={32}/></div>
            <div><p className="text-4xl font-black text-gray-800 dark:text-white">{exams.length}</p><p className="text-xs font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mt-2">Strict Exams</p></div>
          </div>
        </div>
        ...
        {documents.length > 0 && (
          <div className="mb-10 relative">
            ...
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search inside your secure documents..." className="w-full bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl pl-16 pr-6 py-5 text-base font-bold text-gray-800 dark:text-white outline-none focus:border-[var(--accent-color)] shadow-sm transition-colors" />
          </div>
        )}

        {documents.length === 0 ? (
          <div className="border-2 border-dashed border-gray-300 dark:border-zinc-800/80 rounded-[3rem] bg-white/50 dark:bg-zinc-900/20 backdrop-blur-sm p-32 text-center shadow-inner">
            <div className="w-40 h-40 bg-gray-100 dark:bg-zinc-900 rounded-full flex items-center justify-center mx-auto mb-10 shadow-lg">
              <FileUp size={64} className="text-gray-400 dark:text-zinc-600" />
            </div>
            <h2 className="text-3xl font-black text-gray-800 dark:text-white mb-4">Repository Empty</h2>
            <p className="text-gray-500 dark:text-zinc-400 text-lg max-w-xl mx-auto leading-relaxed">Your library is securely encrypted locally. Import a textbook, research paper, or clinical guide to begin.</p>
          </div>
        ) : (
          <div>
            <h3 className="text-xl font-black text-gray-800 dark:text-white mb-8 flex items-center gap-3 uppercase tracking-widest"><Library size={24} className="text-[var(--accent-color)]"/> Local Documents</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {documents.map(doc => {
                ...
                return (
                  <div key={doc.id} onClick={() => onOpen(doc.id)} className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl rounded-[2rem] p-8 border border-gray-200 dark:border-zinc-800 hover:border-[var(--accent-color)]/50 hover:shadow-[0_20px_60px_rgba(var(--accent-color-rgb),0.15)] cursor-pointer transition-all flex flex-col h-72 group relative overflow-hidden shadow-lg">
                    <div className="absolute top-0 right-0 w-40 h-40 bg-[var(--accent-color)]/5 rounded-bl-full -z-10 group-hover:bg-[var(--accent-color)]/10 transition-colors" />
                    <div className="flex items-start justify-between mb-8 z-10">
                      <div className="w-16 h-16 rounded-2xl bg-gray-100 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 flex items-center justify-center text-gray-400 dark:text-zinc-500 group-hover:bg-[var(--accent-color)] group-hover:text-white group-hover:border-[var(--accent-color)] transition-all shadow-md">
                        <BookOpen size={28} />
                      </div>
                      <button onClick={(e) => deleteDocument(doc.id, e)} className="p-3 bg-gray-100 dark:bg-zinc-950/50 text-gray-500 dark:text-zinc-500 hover:text-red-500 hover:bg-red-500/20 rounded-xl transition-all opacity-0 group-hover:opacity-100 backdrop-blur-sm">
                        <Trash2 size={20} />
                      </button>
                    </div>
                    <h3 className="font-black text-gray-800 dark:text-white text-xl leading-snug line-clamp-2 flex-1 z-10">{doc.name}</h3>
                    <div className="mt-4 pt-6 border-t border-gray-200 dark:border-zinc-800/80 z-10">
                      <div className="flex justify-between items-center mb-4">
                        <div className="flex gap-2">
                          <span className="text-[10px] font-bold text-gray-600 dark:text-zinc-400 bg-gray-100 dark:bg-zinc-950 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-800">{docCards} Cards</span>
                          <span className="text-[10px] font-bold text-gray-600 dark:text-zinc-400 bg-gray-100 dark:bg-zinc-950 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-800">{docExams} Exams</span>
                        </div>
                        <span className="text-xs font-black text-[var(--accent-color)] bg-[var(--accent-color)]/10 px-3 py-1.5 rounded-lg border border-[var(--accent-color)]/20">PG {doc.progress}/{doc.totalPages}</span>
                      </div>
                    </div>
                  </div>
                );
...
function FlashcardsGlobalView({ flashcards, setFlashcards, setView }) {
  ...
  if (flashcards.length === 0) return (
    <div className="flex-1 flex flex-col items-center justify-center p-10 bg-transparent text-center">
      <Layers size={80} className="text-gray-300 dark:text-zinc-800 mb-8" />
      <h2 className="text-3xl font-black text-gray-800 dark:text-white mb-4">No Flashcards Created</h2>
      <p className="text-gray-500 dark:text-zinc-400 max-w-md text-lg">Open a document and use the AI Generator to extract high-yield flashcard sets.</p>
      <button onClick={() => setView('library')} className="mt-8 px-8 py-4 bg-[var(--accent-color)] text-white rounded-2xl font-black uppercase tracking-widest shadow-xl hover:-translate-y-1 transition-all">Go to Library</button>
    </div>
  );
    ...
    <div className="flex-1 overflow-auto p-10 lg:p-16 custom-scrollbar bg-transparent">
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          <h1 className="text-4xl font-black text-gray-800 dark:text-white tracking-tighter flex items-center gap-4"><Layers className="text-emerald-500" size={40}/> Global Flashcard Vault</h1>
          <div className="flex gap-4">
            <button onClick={exportAnki} className="px-6 py-4 bg-blue-600 text-white rounded-xl font-bold flex items-center gap-3 shadow-lg hover:-translate-y-1 transition-all uppercase tracking-widest text-xs"><Download size={18} /> Export Anki</button>
            <button onClick={() => setStudyingSet({ id: 'all', title: 'All Flashcards Master Set', cards: flashcards.flatMap(s => s.cards || []) })} className="px-8 py-4 bg-[var(--accent-color)] text-white rounded-xl font-black flex items-center gap-3 shadow-lg shadow-[var(--accent-color)]/30 hover:scale-105 transition-transform uppercase tracking-widest text-xs"><Play size={18} fill="currentColor" /> Study Everything</button>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {flashcards.map(set => (
            <div key={set.id} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-8 rounded-3xl relative group shadow-lg hover:shadow-xl hover:border-[var(--accent-color)]/50 transition-all flex flex-col">
              <div className="flex-1">
                <p className="text-2xl text-gray-800 dark:text-white font-black mb-4 pr-12 leading-snug">{set.title}</p>
                <div className="flex items-center gap-3 mb-8">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[var(--accent-color)] bg-[var(--accent-color)]/10 px-3 py-1.5 rounded-lg border border-[var(--accent-color)]/20">{set.cards ? set.cards.length : 0} Cards</span>
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-zinc-500 bg-gray-100 dark:bg-zinc-950 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-800">Source: Pages {set.sourcePages}</span>
                </div>
              </div>
              <div className="flex gap-4 mt-auto">
                <button onClick={() => setStudyingSet(set)} className="flex-1 py-4 bg-[var(--accent-color)] hover:bg-[var(--accent-color)]/90 text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-md"><Play size={16} fill="currentColor" /> Study Set</button>
                <button onClick={() => setFlashcards(flashcards.filter(f => f.id !== set.id))} className="p-4 bg-gray-100 dark:bg-zinc-950 text-gray-600 dark:text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-2xl transition-all"><Trash2 size={20}/></button>
              </div>
            </div>
          ))}
        </div>
...
function ExamsGlobalView({ exams, setExams, setView }) {
  ...
  if (selectedExam) {
    return (
      <div className="flex-1 flex flex-col bg-transparent">
        <div className="h-20 flex items-center justify-between px-10 bg-white/90 dark:bg-[#121214]/90 backdrop-blur-xl border-b border-gray-200 dark:border-zinc-800 shrink-0 shadow-md z-10">
          <button
            onClick={() => setSelectedExam(null)}
            className="flex items-center gap-3 text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 text-sm font-black uppercase tracking-widest transition-colors bg-emerald-500/10 px-5 py-3 rounded-xl border border-emerald-500/20"
          >
            <ChevronLeft size={20} /> Exit Examination
          </button>
          <span className="text-sm font-bold text-gray-800 dark:text-white uppercase tracking-widest">
            {selectedExam.title} <span className="mx-3 text-gray-300 dark:text-zinc-700">|</span> <span className="text-emerald-500">{selectedExam.questions.length} Items</span>
          </span>
        </div>
  ...
  if (exams.length === 0) return (
    <div className="flex-1 flex flex-col items-center justify-center p-10 bg-transparent text-center">
      <GraduationCap size={80} className="text-gray-300 dark:text-zinc-800 mb-8" />
      <h2 className="text-3xl font-black text-gray-800 dark:text-white mb-4">No Exams Generated</h2>
      <p className="text-gray-500 dark:text-zinc-400 max-w-md text-lg">Open a document and use the AI to generate a strict, board-level test on specific pages.</p>
      <button onClick={() => setView('library')} className="mt-8 px-8 py-4 bg-[var(--accent-color)] text-white rounded-2xl font-black uppercase tracking-widest shadow-xl hover:-translate-y-1 transition-all">Go to Library</button>
    </div>
  );
  return (
    <div className="flex-1 overflow-auto p-10 lg:p-16 custom-scrollbar bg-transparent">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-black text-gray-800 dark:text-white tracking-tighter mb-12 flex items-center gap-4"><GraduationCap className="text-emerald-500" size={40}/> Global Examination Vault</h1>
        <div className="space-y-6">
          {exams.map(e => (
            <div key={e.id} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-8 rounded-[2rem] flex flex-col md:flex-row justify-between items-start md:items-center group hover:border-emerald-500/50 transition-all shadow-lg hover:shadow-xl gap-6">
              <div className="flex-1">
                <p className="text-2xl text-gray-800 dark:text-white font-black mb-3">{e.title}</p>
                <div className="flex items-center gap-3">
                  <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-500/20">{e.questions.length} Questions</span>
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-zinc-500 bg-gray-100 dark:bg-zinc-950 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-800">Source: Pgs {e.sourcePages}</span>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                <button onClick={() => exportExamPdf(e)} className="flex-1 md:flex-none px-5 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-md"><Printer size={18} /> Export PDF</button>
                <button onClick={() => setSelectedExam(e)} className="flex-1 md:flex-none px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/30 hover:scale-105"><Play size={18} fill="currentColor"/> Take Exam</button>
                <button onClick={() => setExams(exams.filter(ex => ex.id !== e.id))} className="p-4 bg-gray-100 dark:bg-zinc-950 text-gray-500 dark:text-zinc-600 hover:text-red-500 hover:bg-red-500/10 rounded-2xl transition-all shadow-sm"><Trash2 size={20}/></button>
              </div>
            </div>
          ))}
        </div>
...
function NotesGlobalView({ notes, setNotes, setView }) {
  if (notes.length === 0) return (
    <div className="flex-1 flex flex-col items-center justify-center p-10 bg-transparent text-center">
      <BookA size={80} className="text-gray-300 dark:text-zinc-800 mb-8" />
      <h2 className="text-3xl font-black text-gray-800 dark:text-white mb-4">No Notes Found</h2>
      <p className="text-gray-500 dark:text-zinc-400 max-w-md text-lg">Open a document and use the AI to generate clinical cases, mnemonics, or summaries.</p>
      <button onClick={() => setView('library')} className="mt-8 px-8 py-4 bg-[var(--accent-color)] text-white rounded-2xl font-black uppercase tracking-widest shadow-xl hover:-translate-y-1 transition-all">Go to Library</button>
    </div>
  );
  return (
    <div className="flex-1 overflow-auto p-10 lg:p-16 custom-scrollbar bg-transparent">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-4xl font-black text-gray-800 dark:text-white tracking-tighter mb-12 flex items-center gap-4"><BookA className="text-blue-500" size={40}/> Global Clinical Notes</h1>
        <div className="space-y-8">
          {notes.map(n => (
            <div key={n.id} className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-10 rounded-[2.5rem] relative group hover:border-blue-500/50 transition-all shadow-xl">
              <h3 className="text-2xl text-gray-800 dark:text-white font-black mb-6 pr-16">{n.title}</h3>
              <div className="text-base text-gray-700 dark:text-zinc-300 whitespace-pre-wrap leading-relaxed prose prose-invert max-w-none prose-headings:text-gray-900 dark:prose-headings:text-white prose-strong:text-[var(--accent-color)]">{n.content}</div>
              <button onClick={() => setNotes(notes.filter(no => no.id !== n.id))} className="absolute top-10 right-10 p-3 bg-gray-100 dark:bg-zinc-950 text-gray-500 dark:text-zinc-600 hover:text-red-500 hover:bg-red-500/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all shadow-md"><Trash2 size={20}/></button>
            </div>
          ))}
        </div>
      </div>
...
function PdfWorkspace({ activeDoc, setDocuments, closeDoc, rightPanelOpen, setRightPanelOpen, currentPage, setCurrentPage, openDocs, setActiveDocId, closeTab, setShowMenu, setMenuPos, setSelectedText, setGenFromSelection, setRightPanelTab }) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gray-200 dark:bg-[#050505] relative" onContextMenu={handleContextMenu}>
      <div className="h-16 flex items-center justify-between px-6 bg-white/95 dark:bg-[#0a0a0c]/95 backdrop-blur-xl border-b border-gray-300 dark:border-zinc-800/50 shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-4 overflow-hidden">
          <button onClick={closeDoc} className="flex items-center gap-2 text-gray-500 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-white transition-colors text-xs font-black uppercase tracking-widest bg-gray-100 dark:bg-zinc-900 px-4 py-2 rounded-xl">
      ...
      {openDocs.length > 1 && (
        <div className="flex gap-2 px-6 py-2 bg-white/95 dark:bg-[#0a0a0c]/95 backdrop-blur-xl border-b border-gray-300 dark:border-zinc-800/50 overflow-x-auto custom-scrollbar">
          {openDocs.map(id => {
            const doc = documents.find(d => d.id === id);
            return (
              <div key={id} className={`flex items-center gap-3 px-5 py-2.5 rounded-xl cursor-pointer transition-all font-bold text-xs uppercase tracking-widest ${activeDocId === id ? 'bg-[var(--accent-color)] text-white shadow-md' : 'bg-gray-100 dark:bg-zinc-900 text-gray-500 dark:text-zinc-500 hover:bg-gray-200 dark:hover:bg-zinc-800 hover:text-gray-800 dark:hover:text-white'}`} onClick={() => setActiveDocId(id)}>
                <span className="truncate max-w-[150px]">{doc.name}</span>
                <button onClick={(e) => { e.stopPropagation(); closeTab(id); }} className={`p-1 rounded-md transition-colors ${activeDocId === id ? 'hover:bg-white/20' : 'hover:bg-gray-300 dark:hover:bg-zinc-700 hover:text-red-500'}`}><X size={14} /></button>
              </div>
            );
          })}
        </div>
      )}
      <div ref={containerRef} className="flex-1 overflow-y-auto bg-gray-200 dark:bg-[#121214] flex flex-col items-center p-8 relative custom-scrollbar">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full gap-6 text-gray-500 dark:text-zinc-500">
            <Loader2 className="animate-spin text-[var(--accent-color)]" size={48}/>
          ...
          <div className="m-auto text-red-500 text-sm flex items-center gap-3 bg-red-50 dark:bg-red-500/10 p-6 rounded-2xl border border-red-200 dark:border-red-500/20 font-bold shadow-sm">
            <AlertCircle size={20}/> Failed to load PDF visual layer. AI text context is still intact.
          </div>
        )}
      </div>
    </div>
...
function PanelSettings({ settings, setSettings }) {
  return (
    <div className="p-8 flex flex-col gap-8">
      <div className="bg-[var(--accent-color)]/10 border border-[var(--accent-color)]/30 p-8 rounded-3xl shadow-lg">
        <h3 className="text-lg font-black text-[var(--accent-color)] flex items-center gap-3 mb-4"><KeyRound size={24}/> OpenAI API Key</h3>
        <p className="text-sm text-gray-600 dark:text-zinc-400 leading-relaxed mb-6">Enter your OpenAI key (sk-...) to power the AI extraction engine. Keys are saved securely in your browser's local storage and never sent to our servers.</p>
        <input
          type="password"
          value={settings.apiKey}
          onChange={(e) => setSettings({...settings, apiKey: e.target.value})}
          placeholder="sk-proj-..."
          className="w-full bg-white dark:bg-[#050505] border border-[var(--accent-color)]/50 rounded-2xl px-6 py-5 text-base text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)] font-mono shadow-inner mb-6 transition-all"
        />
      ...
      <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-8 rounded-3xl shadow-sm">
        <label className="flex items-start gap-5 cursor-pointer">
          <div className="mt-1 relative flex items-center justify-center">
            <input type="checkbox" checked={settings.strictMode} onChange={(e) => setSettings({...settings, strictMode: e.target.checked})} className="peer appearance-none w-6 h-6 border-2 border-gray-300 dark:border-zinc-600 rounded-md bg-gray-50 dark:bg-zinc-950 checked:bg-[var(--accent-color)] checked:border-[var(--accent-color)] transition-all" />
            <CheckSquare size={16} className="text-white absolute pointer-events-none opacity-0 peer-checked:opacity-100" />
          </div>
          <div>
            <span className="text-lg font-bold text-gray-800 dark:text-white block">Strict Document Grounding</span>
            <span className="text-sm text-gray-500 dark:text-zinc-400 mt-2 block leading-relaxed">Forces the AI to exclusively use the text found in the active PDF pages. Essential for medical study to guarantee zero hallucinations.</span>
          </div>
        </label>
      </div>
      <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-8 rounded-3xl shadow-sm">
        <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-3 mb-6"><Moon size={24} className="text-gray-400 dark:text-zinc-500"/> Interface Theme</h3>
        <div className="flex gap-4">
          <button onClick={() => setSettings({...settings, theme: 'system'})} className={`flex-1 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition-all ${settings.theme === 'system' ? 'bg-[var(--accent-color)] text-white shadow-md scale-105' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-zinc-700'}`}>System</button>
          <button onClick={() => setSettings({...settings, theme: 'dark'})} className={`flex-1 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition-all ${settings.theme === 'dark' ? 'bg-[var(--accent-color)] text-white shadow-md scale-105' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-zinc-700'}`}>Dark</button>
          <button onClick={() => setSettings({...settings, theme: 'light'})} className={`flex-1 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition-all ${settings.theme === 'light' ? 'bg-[var(--accent-color)] text-white shadow-md scale-105' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-zinc-700'}`}>Light</button>
        </div>
      </div>
      <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-8 rounded-3xl shadow-sm">
        <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-3 mb-6"><Type size={24} className="text-gray-400 dark:text-zinc-500"/> Typography Scale</h3>
        <div className="flex gap-4">
          <button onClick={() => setSettings({...settings, fontSize: 'small'})} className={`flex-1 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition-all ${settings.fontSize === 'small' ? 'bg-[var(--accent-color)] text-white shadow-md scale-105' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-zinc-700'}`}>Small</button>
          <button onClick={() => setSettings({...settings, fontSize: 'medium'})} className={`flex-1 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition-all ${settings.fontSize === 'medium' ? 'bg-[var(--accent-color)] text-white shadow-md scale-105' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-zinc-700'}`}>Medium</button>
          <button onClick={() => setSettings({...settings, fontSize: 'large'})} className={`flex-1 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition-all ${settings.fontSize === 'large' ? 'bg-[var(--accent-color)] text-white shadow-md scale-105' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-zinc-700'}`}>Large</button>
        </div>
      </div>
      <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-8 rounded-3xl shadow-sm">
        <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-3 mb-6"><Palette size={24} className="text-gray-400 dark:text-zinc-500"/> Brand Accent</h3>
        <div className="flex gap-8 items-center">
          <button onClick={() => setSettings({...settings, accentColor: 'indigo'})} className={`w-14 h-14 rounded-full bg-indigo-500 hover:scale-110 transition-transform ${settings.accentColor === 'indigo' ? 'ring-4 ring-offset-4 ring-indigo-500 dark:ring-offset-zinc-900 shadow-xl' : ''}`} />
          <button onClick={() => setSettings({...settings, accentColor: 'purple'})} className={`w-14 h-14 rounded-full bg-purple-500 hover:scale-110 transition-transform ${settings.accentColor === 'purple' ? 'ring-4 ring-offset-4 ring-purple-500 dark:ring-offset-zinc-900 shadow-xl' : ''}`} />
          <button onClick={() => setSettings({...settings, accentColor: 'blue'})} className={`w-14 h-14 rounded-full bg-blue-500 hover:scale-110 transition-transform ${settings.accentColor === 'blue' ? 'ring-4 ring-offset-4 ring-blue-500 dark:ring-offset-zinc-900 shadow-xl' : ''}`} />
        </div>
      </div>
    </div>
  );
...
function PanelGenerate({ activeDoc, settings, setFlashcards, setExams, setNotes, switchToReview, genFromSelection, setGenFromSelection, currentPage }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setStatus({ loading: true, msg: 'Ultra-Fast parallel processing via AI...', err: false });
      ...
      if (type === 'flashcards' || type === 'exam' || type === 'quiz') {
        ...
        const promises = textChunks.map(async (chunk) => {
          ...
          try {
            ...
            if (lastBrace !== -1) {
               ...
               console.error("JSON Parse Error: Malformed output from AI.");
               return [];
            }
          }
        ...
        let combinedItems = allResults.flat();
        
        combinedItems = combinedItems.slice(0, count);

        if (combinedItems.length === 0) throw new Error("AI failed to extract any items from this text. The text might be irrelevant or too short.");
    ...
    <div className="h-full flex flex-col bg-gray-50 dark:bg-[#0a0a0c] p-6 overflow-y-auto custom-scrollbar">
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-zinc-900 border border-emerald-500/40 rounded-3xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300 shadow-2xl shadow-emerald-900/20">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
            ...
            {generated.type === 'exam' && generated.data.map((item, idx) => (
              <div key={idx} className="bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 p-8 rounded-2xl relative group shadow-sm hover:border-[var(--accent-color)]/30 transition-all">
                <div className="flex justify-between items-start mb-6 gap-6">
                   <p className="text-lg text-gray-800 dark:text-white font-bold leading-relaxed flex-1"><span className="text-gray-400 dark:text-zinc-500 mr-2">{idx+1}.</span> {item.q}</p>
                   <button onClick={()=>removeItem(idx)} className="p-2.5 bg-gray-200 dark:bg-zinc-900 text-gray-500 dark:text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all shrink-0"><Trash2 size={18}/></button>
                </div>
                <div className="space-y-3">
                  {item.options.map((opt, oIdx) => (
                    <div key={oIdx} className={`text-sm p-4 rounded-xl border ${oIdx === item.correct ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-700 dark:text-emerald-400 font-bold' : 'bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-800 text-gray-600 dark:text-zinc-400'}`}>
                      <span className="font-mono text-gray-400 dark:text-zinc-500 mr-3">{String.fromCharCode(65 + oIdx)}.</span> {opt}
                    </div>
                  ))}
                </div>
                <div className="mt-6 p-5 bg-[var(--accent-color)]/5 border border-[var(--accent-color)]/10 rounded-xl">
                  <span className="font-black text-[var(--accent-color)] mr-2 uppercase tracking-widest text-[10px] block mb-3">Explanation</span>
                  <p className="text-sm text-gray-700 dark:text-zinc-300 leading-relaxed font-medium">{item.explanation}</p>
                </div>
              </div>
            ))}
            {generated.type !== 'flashcards' && generated.type !== 'exam' && (
              <div className="bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 p-8 rounded-2xl relative group shadow-sm">
                <div className="text-base text-gray-700 dark:text-zinc-300 whitespace-pre-wrap leading-relaxed prose prose-invert prose-p:text-gray-700 dark:prose-p:text-zinc-300 prose-headings:text-gray-900 dark:prose-headings:text-white max-w-none">{generated.data}</div>
              </div>
            )}
          </div>
        </div>
      ...
      {status.msg && !generated && (
        <div className={`mt-6 p-5 rounded-2xl text-sm font-bold flex items-center gap-4 border ${status.err ? 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20' : 'bg-[var(--accent-color)]/10 text-[var(--accent-color)] border-[var(--accent-color)]/20 shadow-lg shadow-[var(--accent-color)]/20'}`}>
          {status.loading ? <Loader2 size={24} className="animate-spin shrink-0 text-[var(--accent-color)]" /> : <AlertCircle size={24} className="shrink-0 text-red-500" />}
...
function ToolBtn({ id, active, set, icon: Icon, label }) {
  const isA = active === id;
  return (
    <button onClick={() => set(id)} className={`py-5 px-2 flex flex-col items-center justify-center gap-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${isA ? 'bg-[var(--accent-color)] border-[var(--accent-color)] text-white shadow-lg shadow-[var(--accent-color)]/30 scale-105' : 'bg-gray-50 dark:bg-zinc-950 border-gray-200 dark:border-zinc-800 text-gray-500 dark:text-zinc-500 hover:text-gray-800 dark:hover:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800'}`}>
      <Icon size={24} className={isA ? "text-white" : "text-gray-400 dark:text-zinc-600"}/> {label}
    </button>
  );
}
    ...
    <div className="h-full overflow-y-auto custom-scrollbar p-6 bg-gray-50 dark:bg-[#0a0a0c] space-y-12">
            ...
            {docExams.map(e => (
              <div key={e.id} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-6 rounded-3xl group shadow-sm transition-all hover:border-emerald-500/30">
                <div className="flex justify-between items-start mb-5">
                  <div>
                    <p className="text-base text-gray-800 dark:text-white font-bold mb-2">{e.title}</p>
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-zinc-500 bg-gray-100 dark:bg-zinc-950 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-800">{e.questions.length} Questions • Pgs {e.sourcePages}</span>
                  </div>
                  <button onClick={() => setExams(exams.filter(ex => ex.id !== e.id))} className="p-3 bg-gray-100 dark:bg-zinc-950 text-gray-500 dark:text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all shadow-sm"><Trash size={18}/></button>
                </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-5">
           <h3 className="text-[10px] font-black uppercase tracking-widest text-[var(--accent-color)] flex items-center gap-2 bg-[var(--accent-color)]/10 w-fit px-4 py-2 rounded-xl border border-[var(--accent-color)]/20"><Layers size={18}/> Saved Flashcard Sets ({docCardSets.length})</h3>
           {docCardSets.length > 0 && <button onClick={() => setActiveItem({ type: 'flashcards', data: { title: 'All Doc Cards', cards: docCardSets.flatMap(s => s.cards) } })} className="text-[10px] bg-[var(--accent-color)] text-white px-4 py-2 rounded-xl font-bold uppercase tracking-widest flex items-center gap-2 shadow-md hover:bg-[var(--accent-color)]/90 transition-colors"><Play size={14} fill="currentColor"/> Study All</button>}
        </div>
        {docCardSets.length === 0 ? <p className="text-sm text-gray-400 dark:text-zinc-600 italic bg-white dark:bg-zinc-900/50 p-6 rounded-2xl border border-dashed border-gray-200 dark:border-zinc-800 text-center">No flashcards extracted for this document.</p> : (
          <div className="space-y-4">
            {docCardSets.map(set => (
              <div key={set.id} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-6 rounded-3xl group shadow-sm transition-all hover:border-[var(--accent-color)]/30">
                <div className="flex justify-between items-start mb-5">
                  <div>
                    <p className="text-base text-gray-800 dark:text-white font-bold mb-2">{set.title}</p>
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-zinc-500 bg-gray-100 dark:bg-zinc-950 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-800">{set.cards?.length || 0} Cards • Pgs {set.sourcePages}</span>
                  </div>
                  <button onClick={() => setFlashcards(flashcards.filter(f => f.id !== set.id))} className="p-3 bg-gray-100 dark:bg-zinc-950 text-gray-500 dark:text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all shadow-sm"><Trash size={18}/></button>
                </div>
                <button onClick={() => setActiveItem({ type: 'flashcards', data: set })} className="w-full py-3 bg-[var(--accent-color)]/10 hover:bg-[var(--accent-color)]/20 text-[var(--accent-color)] rounded-xl text-xs font-bold uppercase tracking-widest border border-[var(--accent-color)]/20 transition-all flex items-center justify-center gap-2">
                  <Play size={16} fill="currentColor" /> Study Set
                </button>
              </div>
            ))}
          </div>
      ...
      <div>
        <h3 className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-500 mb-5 flex items-center gap-2 bg-blue-500/10 w-fit px-4 py-2 rounded-xl border border-blue-500/20"><BookA size={18}/> Saved Notes ({docNotes.length})</h3>
        {docNotes.length === 0 ? <p className="text-sm text-gray-400 dark:text-zinc-600 italic bg-white dark:bg-zinc-900/50 p-6 rounded-2xl border border-dashed border-gray-200 dark:border-zinc-800 text-center">No notes or summaries generated.</p> : (
          <div className="space-y-4">
            {docNotes.map(n => (
              <div key={n.id} onClick={() => setActiveItem({ type: 'note', data: n })} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 p-6 rounded-3xl group relative shadow-sm hover:border-blue-500/30 transition-all cursor-pointer">
                <div className="pr-14">
                   <p className="text-base text-gray-800 dark:text-white font-bold mb-3">{n.title}</p>
                   <p className="text-sm text-gray-500 dark:text-zinc-400 line-clamp-3 leading-relaxed bg-gray-100 dark:bg-zinc-950 p-4 rounded-2xl border border-gray-200 dark:border-zinc-800">{n.content}</p>
                </div>
                <button onClick={(e) => { e.stopPropagation(); setNotes(notes.filter(no => no.id !== n.id)); }} className="absolute top-6 right-6 p-3 bg-gray-100 dark:bg-zinc-950 text-gray-500 dark:text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all shadow-sm"><Trash size={18}/></button>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-[#0a0a0c]">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
        <>
          <div className="mb-8 flex justify-between items-center border-b border-gray-200 dark:border-zinc-800 pb-4">
          ...
          <h2 className="text-xl text-gray-800 dark:text-white font-bold mb-8 leading-relaxed">{q.q}</h2>
          <div className="space-y-4 mb-8">
            {q.options.map((opt, idx) => (
              ...
              >
                <div className={`w-6 h-6 rounded-full border-2 mt-0.5 flex items-center justify-center shrink-0 ${selectedAnswer === idx || (showFeedback && idx === q.correct) ? (idx === q.correct ? 'border-emerald-500' : 'border-red-500') : 'border-gray-300 dark:border-zinc-600'}`}>
                  {(selectedAnswer === idx || (showFeedback && idx === q.correct)) && <div className={`w-3 h-3 rounded-full ${idx === q.correct ? 'bg-emerald-500' : 'bg-red-500'}`} />}
                </div>
          ...
          <div className="flex justify-between items-center mt-10">
            ...
            <button onClick={nextQuestion} disabled={!showFeedback} className="px-8 py-4 bg-gray-800 dark:bg-white hover:bg-gray-700 dark:hover:bg-zinc-200 disabled:bg-gray-200 dark:disabled:bg-zinc-800 disabled:text-gray-400 dark:disabled:text-zinc-600 text-white dark:text-zinc-950 disabled:shadow-none rounded-xl text-sm font-black shadow-lg uppercase tracking-widest transition-all flex items-center gap-3">Next <ChevronRight size={18}/></button>
          </div>
        </>
      </div>
    </div>
...
function InPanelFlashcards({ title, initialCards, onBack, setFlashcards }) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-[#0a0a0c]">
      <div className="bg-[var(--accent-color)]/10 border-b border-[var(--accent-color)]/20 p-5 flex items-center justify-between shrink-0">
        <button onClick={onBack} className="text-[var(--accent-color)] hover:text-[var(--accent-color)]/80 text-xs font-bold uppercase tracking-widest flex items-center gap-2"><ChevronLeft size={16}/> Exit Study</button>
        <span className="text-[10px] text-[var(--accent-color)] font-black uppercase tracking-widest bg-[var(--accent-color)]/10 px-3 py-1 rounded-lg">Card {currentIndex+1} / {cards.length}</span>
      </div>
        ...
        <div onClick={() => !isFlipped && setIsFlipped(true)} className="w-full h-96 perspective-1000 cursor-pointer">
          <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-x-180' : ''}`}>
            ...
            <div className="absolute inset-0 backface-hidden bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-3xl p-10 flex flex-col items-center justify-center text-center shadow-xl">
              ...
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white leading-relaxed">{currentCard.q}</h2>
              <div className="absolute bottom-6 text-[10px] font-bold uppercase tracking-widest text-[var(--accent-color)]/50 animate-pulse">Click card to reveal answer</div>
            </div>
            {/* BACK */}
            <div className="absolute inset-0 backface-hidden bg-[var(--accent-color)] border border-[var(--accent-color)]/50 rounded-3xl p-10 flex flex-col items-center justify-center text-center shadow-2xl rotate-x-180 overflow-y-auto custom-scrollbar">
              ...
              <p className="text-xl font-bold text-white leading-relaxed">{currentCard.a}</p>
            </div>
          </div>
        </div>
        <div className={`w-full flex gap-4 mt-12 transition-opacity duration-300 ${isFlipped ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <button onClick={() => handleRate(0)} className="flex-1 py-5 bg-red-500/10 border border-red-500/20 hover:bg-red-500 hover:text-white rounded-2xl text-red-600 dark:text-red-400 text-sm font-black uppercase tracking-widest transition-all shadow-sm">Again</button>
          <button onClick={() => handleRate(3)} className="flex-1 py-5 bg-yellow-500/10 border border-yellow-500/20 hover:bg-yellow-500 hover:text-white rounded-2xl text-yellow-600 dark:text-yellow-400 text-sm font-black uppercase tracking-widest transition-all shadow-sm">Hard</button>
          <button onClick={() => handleRate(4)} className="flex-1 py-5 bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500 hover:text-white rounded-2xl text-emerald-600 dark:text-emerald-400 text-sm font-black uppercase tracking-widest transition-all shadow-sm">Good</button>
          <button onClick={() => handleRate(5)} className="flex-1 py-5 bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500 hover:text-white rounded-2xl text-blue-600 dark:text-blue-400 text-sm font-black uppercase tracking-widest transition-all shadow-sm">Easy</button>
        </div>
      </div>
...
function InPanelNote({ note, onBack }) {
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-[#0a0a0c]">
      <div className="bg-blue-600/10 border-b border-blue-500/20 p-5 flex items-center justify-between shrink-0">
        <button onClick={onBack} className="text-blue-600 dark:text-blue-400 hover:text-blue-500 text-xs font-bold uppercase tracking-widest flex items-center gap-2"><ChevronLeft size={16}/> Back to Notes</button>
      </div>
      <div className="flex-1 overflow-y-auto custom-scrollbar p-10">
         <h2 className="text-3xl font-black text-gray-800 dark:text-white mb-8 leading-snug">{note.title}</h2>
         <div className="prose prose-invert prose-lg max-w-none prose-p:text-gray-600 dark:prose-p:text-zinc-300 prose-headings:text-gray-800 dark:prose-headings:text-white prose-strong:text-[var(--accent-color)] whitespace-pre-wrap leading-relaxed">
            {note.content}
         </div>
      </div>
    </div>
