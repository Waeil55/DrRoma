...
{result}
{flag && (
<span className={`text-[8px] font-black px-1 py-0.5 rounded leading-none ${isLow ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300' : 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300'}`}>{flag}</span>
)}
</span>
</td>
...
{toasts.map(t => (
<div key={t.id} className={`px-4 py-3 rounded-2xl text-xs font-bold shadow-2xl backdrop-blur-xl border flex items-center gap-2 animate-slide-in pointer-events-auto ${t.type === 'success' ? 'bg-emerald-500/90 text-white border-emerald-400/30' : t.type === 'error' ? 'bg-red-500/90 text-white border-red-400/30' : 'bg-white/90 dark:bg-zinc-900/90 text-gray-800 dark:text-white border-gray-200 dark:border-zinc-700'}`}>
...
```
  let allCases = [];
  await runParallel(tasks, 5, (done, total) => {
...
<p className="text-sm font-bold text-[var(--accent-color)] mb-3">{selectedCase.diagnosis}</p>
...
<div className="mb-6">
<h3 className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-3 flex items-center gap-2"><FlaskConical size={14}/> Laboratory Results</h3>
...
<h3 className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 mb-3 flex items-center gap-2"><CheckSquare size={14}/> Exam Question</h3>
...
{status.loading && <div className="absolute inset-0 bg-rose-500/20 animate-pulse"/>}
</div>
<div>
<h2 className="text-base font-black text-gray-800 dark:text-white">Turbo Case Generator</h2>
  ...
  {!cases && (
    <>
      <div className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 rounded-2xl p-5 mb-4 shadow-sm">
        ...
        <input type="range" min="1" max="100" value={caseCount} onChange={e => setCaseCount(parseInt(e.target.value))} className="w-full accent-rose-500 mb-6"/>
      ...
      <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/30 rounded-xl p-4 mb-4">
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-blue-600 dark:text-blue-500 leading-relaxed">Realistic patient vignettes with complete lab panels (CBC, BMP, disease-specific) in table format with flags for abnormal values, plus MCQs.</p>
          </div>
        </div>
      </div>
      {status.loading && progress.total > 0 && (
        <div className="mb-4 bg-gray-50 dark:bg-zinc-900 rounded-xl p-3 border border-gray-200 dark:border-zinc-800">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Progress</span>
            <span className="text-[10px] font-black text-rose-500">{progress.done}/{progress.total}</span>
          </div>
          <div className="w-full bg-gray-200 dark:bg-zinc-800 rounded-full h-1.5">
            <div className="bg-rose-500 h-1.5 rounded-full transition-all duration-300" style={{ width: `${progress.total ? (progress.done / progress.total) * 100 : 0}%` }}/>
          </div>
        </div>
      )}
        ...
        {cases.map((c, i) => (
          <button key={c.id || i} onClick={() => setSelectedCase(c)}
            ...
            <div className="mt-3 pt-3 border-t border-gray-100 dark:border-zinc-800 flex items-center gap-2 flex-wrap">
              <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded-md border border-emerald-100 dark:border-emerald-800">Includes Exam Q</span>
...
<div key={label} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-4 flex items-center gap-3 shadow-sm">
<div className={`w-10 h-10 rounded-xl ${bg} flex items-center justify-center shrink-0`}>
<Icon size={18} className={color}/>
</div>
<div>
<p className="text-xl font-black text-gray-800 dark:text-white leading-none">{value}</p>
<p className="text-[9px] font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mt-0.5">{label}</p>
</div>
</div>
))}
</div>
...
<div className="w-16 h-16 rounded-2xl bg-[var(--accent-color)]/20 flex items-center justify-center">
<BrainCircuit className="w-8 h-8 text-[var(--accent-color)] animate-pulse"/>
</div>
<div className="absolute inset-0 rounded-2xl border-2 border-[var(--accent-color)]/30 animate-ping"/>
</div>
<span className="text-xs font-black uppercase tracking-[0.3em] text-gray-500 dark:text-zinc-500">Loading Intelligence Nexus</span>
<div className="mt-4 flex gap-1">
{[0, 1, 2].map(i => <div key={i} className=“w-1.5 h-1.5 rounded-full bg-[var(–accent-color)]/50 animate-bounce” style={{ animationDelay: `${i * 0.15}s` }}/>)}
</div>
</div>
);
}
...
return (
<>
<style>{`:root { --bg-root: ${isDark ? '#050505' : '#f9fafb'}; --text-main: ${isDark ? '#e4e4e7' : '#1f2937'}; } html, body, #root { margin: 0 !important; padding: 0 !important; width: 100vw !important; height: 100vh !important; overflow: hidden !important; background-color: var(--bg-root); color: var(--text-main); font-family: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; text-align: left !important; } * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; } .custom-scrollbar::-webkit-scrollbar { width: 4px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(128,128,128,0.2); border-radius: 2px; } .pdf-text-layer { position: absolute; inset: 0; overflow: hidden; opacity: 1; line-height: 1; } .pdf-text-layer span { position: absolute; color: transparent; transform-origin: 0% 0%; white-space: pre; cursor: text; } .pdf-text-layer ::selection { background: rgba(var(--accent-color-rgb), 0.3); } canvas { width: 100% !important; height: auto !important; display: block; } @keyframes slide-in { from { opacity: 0; transform: translateX(20px); } to { opacity: 1; transform: translateX(0); } } .animate-slide-in { animation: slide-in 0.2s ease-out; } @keyframes shimmer { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } } .shimmer { background: linear-gradient(90deg, transparent 25%, rgba(var(--accent-color-rgb),0.1) 50%, transparent 75%); background-size: 200% 100%; animation: shimmer 2s infinite; }`}</style>
...
```
  ...
  <div className="flex flex-col md:flex-row h-full w-full overflow-hidden relative">
    ...
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-md h-[76px] z-[100] bg-white/80 dark:bg-[#121214]/80 backdrop-blur-xl border border-gray-200/50 dark:border-zinc-800/80 rounded-[2.5rem] shadow-[0_10px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.7)] flex flex-row items-center justify-around px-2
      md:relative md:transform-none md:left-0 md:bottom-0 md:w-24 md:h-full md:max-w-none md:z-40 md:bg-white md:dark:bg-[#0a0a0c] md:border-y-0 md:border-l-0 md:border-r md:border-gray-200/50 md:dark:border-zinc-800/50 md:rounded-none md:flex-col md:justify-start md:pt-8 md:pb-6 md:px-0 md:shadow-2xl">
      ...
      <div className="flex-1 flex flex-row md:flex-col gap-1 md:gap-6 w-full items-center justify-around md:justify-start px-2 md:px-0">
        ...
        <SidebarBtn icon={Settings} label="Config" active={currentView === 'settings'} onClick={() => setCurrentView('settings')} className="md:hidden"/>
      </div>
    ...
    <main className="flex-1 flex flex-col relative bg-gray-50 dark:bg-[#0a0a0c] min-w-0 h-full overflow-hidden">
      {isUploading && (
        <div className="absolute top-0 left-0 w-full h-1.5 bg-gray-200 dark:bg-zinc-800 z-50">
          <div className="h-full bg-[var(--accent-color)] transition-all duration-300 shadow-[0_0_15px_var(--accent-color)]" style={{ width: `${uploadProgress}%` }}/>
        </div>
      )}
    ...
    {/* Resize Handle */}
    {activeDocId && currentView === 'reader' && rightPanelOpen && (
      <div className="flex w-2 hover:w-3 cursor-col-resize bg-gray-200 dark:bg-zinc-800 hover:bg-[var(--accent-color)] dark:hover:bg-[var(--accent-color)] items-center justify-center shrink-0 z-[120]"
        onMouseDown={(e) => { e.preventDefault(); setIsResizing(true); }}
        onTouchStart={() => setIsResizing(true)}
        style={{ transition: isResizing ? 'none' : 'background-color 0.2s, width 0.2s', backgroundColor: isResizing ? 'var(--accent-color)' : '', width: isResizing ? '8px' : '' }}>
        ...
        <div className="flex-1 overflow-hidden relative flex flex-col min-h-0" style={{ pointerEvents: isResizing ? 'none' : 'auto' }}>
          {rightPanelTab === 'settings' ? (
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8 flex flex-col items-center justify-center text-center pb-[100px] md:pb-6">
              ...
              <p className="text-sm text-gray-500 dark:text-zinc-400 mb-8 leading-relaxed max-w-sm">Connect your OpenAI API key to unlock AI extraction and generation tools.</p>
      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[120] backdrop-blur-sm" onClick={() => setShowShortcuts(false)}>
        <div className="bg-white dark:bg-zinc-900 p-8 md:p-10 rounded-3xl max-w-md w-full shadow-2xl m-4" onClick={e => e.stopPropagation()}>
          ...
          <ul className="space-y-4 text-sm text-gray-600 dark:text-zinc-300">
            {[['Arrow Left / Right', 'Change Page'], ['Ctrl +', 'Zoom In'], ['Ctrl -', 'Zoom Out'], ['?', 'Show Help'], ['Esc', 'Close Modal']].map(([k, v]) => (
              <li key={k} className="flex justify-between items-center border-b border-gray-100 dark:border-zinc-800 pb-2">
                <span>{v}</span><kbd className="bg-gray-100 dark:bg-zinc-800 px-2 py-1 rounded font-mono text-xs font-bold">{k}</kbd>
              </li>
            ))}
          </ul>
    ...
    {showMenu && (
      <div className="fixed inset-0 z-[120]" onClick={() => setShowMenu(false)}>
        <div className="absolute bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-2xl p-2 min-w-[200px]" style={{ left: menuPos.x, top: menuPos.y }}>
          ...
          <button onClick={() => { setShowMenu(false); setGenFromSelection(selectedText); setRightPanelOpen(true); setRightPanelTab('chat'); }} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-xl w-full text-left text-sm font-bold text-blue-500 transition-colors">
...
```
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-16">
      <div onClick={() => setView('chat')} className="cursor-pointer hover:-translate-y-1 transition-all bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md border border-gray-200 dark:border-zinc-800/60 p-5 md:p-8 rounded-2xl md:rounded-[2rem] flex items-center gap-4 md:gap-6 shadow-md hover:shadow-lg group">
        <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-blue-500/10 text-blue-500 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform"><MessageSquare size={24}/></div>
      ...
      <div onClick={() => setView('flashcards')} className="cursor-pointer hover:-translate-y-1 transition-all bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md border border-gray-200 dark:border-zinc-800/60 p-5 md:p-8 rounded-2xl md:rounded-[2rem] flex items-center gap-4 md:gap-6 shadow-md hover:shadow-lg group relative overflow-hidden">
        <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform"><Layers size={24}/></div>
        <div>
          <p className="text-2xl md:text-4xl font-black text-gray-800 dark:text-white">{totalCardsCount}</p>
          <p className="text-[10px] md:text-xs font-bold text-gray-500 dark:text-zinc-500 uppercase tracking-widest mt-1 md:mt-2">Active Flashcards</p>
        </div>
        {dueCards > 0 && <div className="absolute top-3 right-3 bg-red-500 text-white text-[9px] font-black px-2 py-1 rounded-full animate-pulse">{dueCards} Due</div>}
      </div>
      <div onClick={() => setView('exams')} className="cursor-pointer hover:-translate-y-1 transition-all bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md border border-gray-200 dark:border-zinc-800/60 p-5 md:p-8 rounded-2xl md:rounded-[2rem] flex items-center gap-4 md:gap-6 shadow-md hover:shadow-lg group">
        <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-purple-500/10 text-purple-500 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform"><GraduationCap size={24}/></div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
          {filteredDocuments.map(doc => {
            ...
            const docNotes = notes.filter(n => n.docId === doc.id).length;
            const progress = doc.totalPages ? Math.round((doc.progress / doc.totalPages) * 100) : 0;
            return (
                ...
                <div className="flex items-start justify-between mb-6 md:mb-8 z-10">
                  <div className="relative">
                    ...
                    <div className="absolute -bottom-1 -right-1">
                      <ProgressRing value={doc.progress} max={doc.totalPages} size={20} stroke={3}/>
                    </div>
                  </div>
...
if (studyingSet) return <div className="flex-1 overflow-hidden h-full"><InPanelFlashcards title={studyingSet.title} initialCards={studyingSet.cards} onBack={() => setStudyingSet(null)} setFlashcards={setFlashcards} addToast={addToast}/></div>;
if (flashcards.length === 0) return (
<div className="flex-1 flex flex-col items-center justify-center p-6 text-center h-full">
<Layers size={80} className="text-gray-300 dark:text-zinc-800 mb-6"/>
<h2 className="text-2xl font-black text-gray-800 dark:text-white mb-2">No Flashcards Yet</h2>
<p className="text-gray-500 dark:text-zinc-500 text-sm mb-6">Upload a document and generate flashcards from the Reader.</p>
<button onClick={() => setView(‘library’)} className=“mt-2 px-8 py-3 bg-[var(–accent-color)] text-white rounded-xl font-black uppercase tracking-widest shadow-xl hover:-translate-y-1 transition-all”>Go to Library</button>
</div>
);
...
<p className="text-sm text-gray-500 dark:text-zinc-500 mt-1">{flashcards.reduce((s,f) => s+(f.cards?.length||0),0)} total cards across {flashcards.length} sets</p>
</div>
<div className="flex gap-3">
<button onClick={exportAnki} className="px-4 py-2.5 bg-blue-600 text-white rounded-xl font-bold flex items-center gap-2 shadow-lg text-xs uppercase tracking-widest"><Download size={14}/> Anki</button>
{dueCards.length > 0 && (
<button onClick={() => setStudyingSet({ id: ‘due’, title: ‘Due Cards’, cards: dueCards })} className=“px-4 py-2.5 bg-red-500 text-white rounded-xl font-bold flex items-center gap-2 shadow-lg text-xs uppercase tracking-widest animate-pulse”>
<Clock size={14}/> {dueCards.length} Due
</button>
)}
<button onClick={() => setStudyingSet({ id: ‘all’, title: ‘All Cards’, cards: flashcards.flatMap(s => s.cards || []) })} className=“px-5 py-2.5 bg-[var(–accent-color)] text-white rounded-xl font-black flex items-center gap-2 shadow-lg text-xs uppercase tracking-widest”><Play size={14} fill="currentColor"/> Study All</button>
</div>
</div>
...
```
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      {flashcards.map(set => {
        ...
        return (
          <div key={set.id} className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 p-6 rounded-3xl shadow-lg flex flex-col hover:border-[var(--accent-color)]/30 transition-all">
            <div className="flex items-start justify-between mb-3">
              <p className="text-xl font-black text-gray-800 dark:text-white flex-1 pr-2">{set.title}</p>
              {due > 0 && <span className="text-[9px] font-black text-red-500 bg-red-500/10 px-2 py-1 rounded-lg border border-red-500/20 shrink-0">{due} due</span>}
            </div>
            <div className="flex gap-2 mb-3">
              <span className="text-[10px] font-black text-[var(--accent-color)] bg-[var(--accent-color)]/10 px-3 py-1.5 rounded-lg border border-[var(--accent-color)]/20">{set.cards?.length || 0} Cards</span>
              <span className="text-[10px] font-black text-emerald-600 bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-500/20">{mastered} Mastered</span>
            </div>
            {/* Progress bar */}
            <div className="mb-4">
              <div className="w-full bg-gray-100 dark:bg-zinc-800 rounded-full h-1.5">
                <div className="bg-emerald-500 h-1.5 rounded-full transition-all" style={{ width: set.cards?.length ? `${(mastered / set.cards.length) * 100}%` : '0%' }}/>
              </div>
              <p className="text-[9px] text-gray-400 mt-1">{set.cards?.length ? Math.round((mastered / set.cards.length) * 100) : 0}% mastered</p>
            </div>
            <div className="flex gap-3 mt-auto">
              ...
              {due > 0 && <button onClick={() => setStudyingSet({ ...set, cards: (set.cards || []).filter(c => c.nextReview <= Date.now()) })} className="py-3 px-3 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl font-black text-xs flex items-center gap-1"><Clock size={13}/> {due}</button>}
...
<div className="h-16 flex items-center justify-between px-6 bg-white/90 dark:bg-[#121214]/90 border-b border-gray-200 dark:border-zinc-800 shrink-0">
<button onClick={() => setSelectedExam(null)} className=“flex items-center gap-2 text-emerald-600 dark:text-emerald-400 text-xs font-black uppercase tracking-widest bg-emerald-500/10 px-4 py-2 rounded-xl border border-emerald-500/20”><ChevronLeft size={18}/> Exit Exam</button>
<span className="text-xs font-bold text-gray-600 dark:text-zinc-400">{selectedExam.title}</span>
</div>
<div className="flex-1 overflow-hidden relative"><InPanelExam exam={selectedExam} onBack={() => setSelectedExam(null)} onScoreUpdate={(id, score) => setExams(prev => prev.map(e => e.id === id ? { …e, lastScore: score } : e))} addToast={addToast}/></div>
</div>
);
if (exams.length === 0) return (
<div className="flex-1 flex flex-col items-center justify-center p-6 text-center h-full">
<GraduationCap size={80} className="text-gray-300 dark:text-zinc-800 mb-6"/>
<h2 className="text-2xl font-black text-gray-800 dark:text-white mb-2">No Exams Yet</h2>
<p className="text-gray-500 dark:text-zinc-500 text-sm mb-6">Generate exams from your documents in the Reader.</p>
<button onClick={() => setView(‘library’)} className=“mt-2 px-8 py-3 bg-[var(–accent-color)] text-white rounded-xl font-black uppercase tracking-widest shadow-xl hover:-translate-y-1 transition-all”>Go to Library</button>
</div>
);
return (
...
<div className="flex-1">
<p className="text-xl font-black text-gray-800 dark:text-white mb-2">{e.title}</p>
<div className="flex gap-2 flex-wrap">
<span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-500/20">{e.questions.length} Questions</span>
{e.lastScore !== undefined && (
<span className={`text-[10px] font-black px-3 py-1.5 rounded-lg border ${e.lastScore >= 70 ? 'text-emerald-600 bg-emerald-500/10 border-emerald-500/20' : 'text-red-600 bg-red-500/10 border-red-500/20'}`}>
Last Score: {e.lastScore}%
</span>
)}
</div>
</div>
<div className="flex gap-3">
<button onClick={() => setSelectedExam(e)} className=“px-6 py-3 bg-emerald-600 text-white rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-2 shadow-lg hover:bg-emerald-500 transition-colors”><Play size={16} fill="currentColor"/> Take Exam</button>
<button onClick={() => setExams(exams.filter(ex => ex.id !== e.id))} className=“p-3 bg-gray-100 dark:bg-zinc-950 text-gray-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-colors”><Trash2 size={18}/></button>
</div>
</div>
))}
</div>
...
<div className="truncate pr-6 text-sm">{session.title}</div>
<div className="text-[10px] text-gray-400 dark:text-zinc-600 mt-0.5">{session.messages.length} messages</div>
</button>
...
```
  ...
  <div className="flex-1 flex flex-col h-full relative">
    <div className="h-16 border-b border-gray-200 dark:border-zinc-800 bg-white/95 dark:bg-[#0a0a0c]/95 flex items-center justify-between px-4 md:px-6 shrink-0 z-10">
      <div className="flex items-center gap-3">
        ...
        <span className="font-bold text-gray-800 dark:text-white text-sm md:text-base">Smart Global Chat</span>
      </div>
      <div className="flex items-center gap-2">
        {!activeSessionId && (
          <select value={selectedDocId} onChange={e => setSelectedDocId(e.target.value)} className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 text-gray-800 dark:text-zinc-300 text-xs font-bold rounded-lg px-3 py-1.5 outline-none max-w-[140px] md:max-w-xs truncate">
            <option value="none">No Document</option>
            {documents.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
          </select>
        )}
        {activeSessionId && activeSession?.docId && <span className="text-[10px] font-black uppercase tracking-widest text-[var(--accent-color)] bg-[var(--accent-color)]/10 px-2 py-1 rounded-md flex items-center gap-1"><BookOpen size={12}/> Doc Attached</span>}
        {activeSessionId && <button onClick={createNewChat} className="p-2 bg-gray-100 dark:bg-zinc-900 rounded-lg text-gray-500 hover:text-gray-800 dark:hover:text-white transition-colors border border-gray-200 dark:border-zinc-800"><PlusCircle size={16}/></button>}
      </div>
    </div>

    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 pb-[160px] md:pb-[120px] flex flex-col">
      {!activeSession ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center opacity-70">
          <div className="w-20 h-20 rounded-3xl bg-[var(--accent-color)]/10 flex items-center justify-center mb-6">
            <Sparkles size={40} className="text-[var(--accent-color)]"/>
          </div>
          <h2 className="text-2xl font-black text-gray-800 dark:text-white mb-2">How can I help?</h2>
          <p className="text-gray-500 max-w-sm text-sm">Attach a document above to ground the AI in specific clinical text, or chat globally for elite medical insights.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-8 max-w-md w-full text-left">
            {['Explain the pathophysiology of heart failure', 'What are the diagnostic criteria for sepsis?', 'Summarize the treatment of T2DM', 'What labs suggest acute pancreatitis?'].map(q => (
              <button key={q} onClick={() => { setInput(q); }} className="p-3 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl text-xs text-gray-600 dark:text-zinc-400 font-medium text-left hover:border-[var(--accent-color)]/50 hover:text-[var(--accent-color)] transition-all">{q}</button>
            ))}
          </div>
        </div>
      ) : (
        <div className="max-w-3xl mx-auto w-full space-y-6">
          {activeSession.messages.map((m, i) => (
            <div key={i} className={`flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`p-5 text-sm leading-relaxed shadow-sm max-w-[85%] ${m.role === 'user' ? 'bg-[var(--accent-color)] text-white rounded-[1.5rem] rounded-tr-sm' : 'bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-[1.5rem] rounded-tl-sm text-gray-700 dark:text-zinc-300 w-full whitespace-pre-wrap'}`}>
                {m.content}
              </div>
            </div>
          ))}
          {loading && (
            <div className="flex gap-4">
              ...
              <div className="p-5 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-[1.5rem] rounded-tl-sm flex gap-1.5 items-center">
    ...
    {/* ✅ FIX: Input box above mobile nav — bottom-24 on mobile, bottom-6 on desktop */}
    <div className="absolute bottom-24 md:bottom-6 left-0 right-0 px-4 md:px-8">
      <div className="max-w-3xl mx-auto bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.05)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.5)] focus-within:border-[var(--accent-color)] focus-within:ring-2 focus-within:ring-[var(--accent-color)]/20 transition-all p-2">
        <textarea ref={textareaRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="Ask anything, anytime..." disabled={loading} className="w-full bg-transparent p-4 pr-16 text-sm text-gray-800 dark:text-white outline-none resize-none max-h-40 custom-scrollbar" style={{ minHeight: '60px' }}/>
        <button onClick={handleSend} disabled={loading || !input.trim()} className="absolute right-6 bottom-6 p-3 bg-[var(--accent-color)] disabled:bg-gray-200 dark:disabled:bg-zinc-800 rounded-xl text-white disabled:text-gray-400 transition-all hover:scale-105 active:scale-95 shadow-lg">
          <Send size={18}/>
        </button>
      </div>
    </div>
...
<div className="w-px h-6 bg-gray-300 dark:bg-zinc-800"/>
<span className="text-sm font-bold text-gray-800 dark:text-zinc-200 truncate max-w-[150px] md:max-w-md">{activeDoc.name}</span>
</div>
<div className="flex items-center gap-2">
<div className="hidden md:flex items-center gap-1 bg-gray-100 dark:bg-zinc-900 p-1 rounded-xl border border-gray-200 dark:border-zinc-800 mr-1">
...
<div className="flex items-center gap-1 bg-gray-100 dark:bg-zinc-900 p-1 rounded-xl border border-gray-200 dark:border-zinc-800 mr-1">
...
<button onClick={handleZoomReset} className="p-2 text-gray-500 hover:text-gray-900 dark:hover:text-white rounded-lg hover:bg-gray-200 dark:hover:bg-zinc-800 transition-colors" title="Fit Width"><Maximize size={16}/></button>
...
```
  ...
  {openDocs.length > 1 && (
    <div className="flex gap-2 px-4 py-2 bg-white/95 dark:bg-[#0a0a0c]/95 border-b border-gray-200 dark:border-zinc-800/30 overflow-x-auto custom-scrollbar shrink-0">
      {openDocs.map(id => {
        ...
        if (!doc) return null;
        return (
          <div key={id} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg cursor-pointer font-bold text-[9px] uppercase tracking-widest ${id === activeDocId ? 'bg-[var(--accent-color)] text-white' : 'bg-gray-100 dark:bg-zinc-900 text-gray-500 hover:bg-gray-200 dark:hover:bg-zinc-800'}`} onClick={() => setActiveDocId(id)}>
  ...
  <div ref={containerRef} className="flex-1 overflow-auto bg-gray-200 dark:bg-[#121214] block relative custom-scrollbar p-0 m-0">
    {isLoading ? (
      <div className="flex flex-col items-center justify-center h-full gap-4 text-gray-500 dark:text-zinc-500 min-h-[50vh]">
        <Loader2 className="animate-spin text-[var(--accent-color)]" size={32}/>
        <span className="text-xs font-black tracking-[0.2em] uppercase">Rendering Viewer...</span>
      </div>
    ) : pdf ? (
      <div className="relative shadow-xl dark:shadow-[0_20px_60px_rgba(0,0,0,0.8)] bg-white shrink-0 origin-top-left m-0 p-0"
      ...
      <div className="m-auto text-red-500 text-sm flex items-center gap-2 bg-red-50 dark:bg-red-500/10 p-6 rounded-2xl border border-red-200 dark:border-red-500/20 font-bold min-h-[50vh] mt-10 w-fit"><AlertCircle size={16}/> Failed to load PDF. Please re-import.</div>
    )}
  </div>

  {/* ✅ FIX: Navigation Bar — z-20, pb-[100px] on mobile so buttons clear mobile nav */}
  <div className="py-3 md:h-[72px] flex items-center justify-center gap-4 bg-white dark:bg-[#0a0a0c] border-t border-gray-200 dark:border-zinc-800/50 shrink-0 z-20 w-full pb-[100px] md:pb-0">
    {/* Back Button */}
    <button
      onClick={() => handleNav(-1)}
      disabled={localPage <= 1}
      className="p-3 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed text-gray-800 dark:text-white rounded-[1rem] transition-colors border border-gray-200 dark:border-zinc-700 shadow-sm active:scale-95"
    >
      <ChevronLeft size={20}/>
    </button>

    {/* Page indicator — tap to go to page */}
    <button
      onClick={() => setShowGoTo(true)}
      className="px-4 py-2 font-bold text-gray-800 dark:text-white font-mono tracking-widest text-sm whitespace-nowrap bg-gray-50 dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 hover:border-[var(--accent-color)] transition-colors"
    >
      PG <span className="text-[var(--accent-color)]">{localPage}</span> / {activeDoc.totalPages}
    </button>

    {/* Next Button */}
    <button
      onClick={() => handleNav(1)}
      disabled={localPage >= activeDoc.totalPages}
      className="p-3 bg-[var(--accent-color)] hover:bg-[var(--accent-color)]/90 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-[1rem] transition-colors shadow-md shadow-[var(--accent-color)]/30 active:scale-95"
    >
  ...
  {/* Go To Page Modal */}
  {showGoTo && (
    <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-50 backdrop-blur-sm" onClick={() => setShowGoTo(false)}>
      <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-800 w-64" onClick={e => e.stopPropagation()}>
        <h3 className="text-sm font-black text-gray-800 dark:text-white mb-4 uppercase tracking-widest">Go to Page</h3>
        <input type="number" min="1" max={activeDoc.totalPages} value={goToPage} onChange={e => setGoToPage(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleGoToPage()} className="w-full bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-700 rounded-xl px-4 py-3 text-center text-gray-800 dark:text-white font-mono font-bold text-sm outline-none focus:border-[var(--accent-color)] mb-4" placeholder={`1 – ${activeDoc.totalPages}`} autoFocus/>
        <div className="flex gap-2">
          <button onClick={() => setShowGoTo(false)} className="flex-1 py-2 bg-gray-100 dark:bg-zinc-800 rounded-xl text-xs font-bold text-gray-600 dark:text-zinc-300">Cancel</button>
          <button onClick={handleGoToPage} className="flex-1 py-2 bg-[var(--accent-color)] rounded-xl text-xs font-bold text-white">Go</button>
        </div>
      </div>
    </div>
  )}
...
<span className="text-[10px] text-gray-400 font-bold">{({ small:‘14px’, medium:‘16px’, large:‘18px’, xl:‘20px’, xxl:‘24px’, xxxl:‘28px’ })[settings.fontSize]}</span>
</div>
...
```
  ...
  <div className="bg-[var(--accent-color)]/5 dark:bg-[var(--accent-color)]/10 border border-[var(--accent-color)]/20 p-6 rounded-[2rem] shadow-lg">
    ...
    <div className="relative mb-4">
      <input type={showKey ? 'text' : 'password'} value={settings.apiKey} onChange={(e) => setSettings({ ...settings, apiKey: e.target.value })} placeholder="sk-proj-..." className="w-full bg-white dark:bg-[#050505] border border-gray-200 dark:border-[var(--accent-color)]/50 rounded-2xl px-5 py-4 pr-12 text-sm text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)] font-mono shadow-inner transition-all"/>
      <button onClick={() => setShowKey(!showKey)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 dark:hover:text-white transition-colors">
        {showKey ? <EyeOff size={16}/> : <Eye size={16}/>}
      </button>
    </div>
    <div className="flex gap-3">
      <button onClick={testApiKey} disabled={testingKey || !settings.apiKey?.trim()} className="px-4 py-2 bg-[var(--accent-color)] text-white rounded-xl text-xs font-bold flex items-center gap-2 disabled:opacity-50 hover:bg-[var(--accent-color)]/90 transition-colors">
        {testingKey ? <Loader2 size={14} className="animate-spin"/> : <CheckCircle2 size={14}/>} Test Key
      </button>
      <a href="https://platform.openai.com/api-keys" target="_blank" rel="noreferrer" className="px-4 py-2 bg-white/50 dark:bg-black/20 text-[var(--accent-color)] rounded-xl text-xs font-bold flex items-center gap-2 border border-[var(--accent-color)]/20 hover:bg-[var(--accent-color)]/10 transition-colors">Get Key <ChevronRight size={12}/></a>
    </div>
  </div>
  ...
  <div className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 p-6 rounded-[2rem]">
    <label className="flex items-start gap-4 cursor-pointer">
      ...
      <div>
        ...
        <span className="text-sm text-gray-500 dark:text-zinc-400 mt-1 block leading-relaxed">Forces AI to use only the text from the PDF. Zero hallucinations.</span>
      </div>
    </label>
  </div>
  ...
  <div className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 p-6 rounded-[2rem]">
    ...
    <div className="flex gap-3">
      {[['system', 'System'], ['dark', 'Dark'], ['light', 'Light']].map(([t, l]) => (
        <button key={t} onClick={() => setSettings({ ...settings, theme: t })} className={`flex-1 py-3 rounded-2xl font-bold uppercase tracking-widest text-xs transition-all ${settings.theme === t ? 'bg-[var(--accent-color)] text-white shadow-md' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 hover:bg-gray-200'}`}>{l}</button>
      ))}
    </div>
  </div>
  ...
  <div className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 p-6 rounded-[2rem]">
    ...
    <div className="flex gap-4 items-center">
      {[['indigo', 'bg-indigo-500', 'ring-indigo-500'], ['purple', 'bg-purple-500', 'ring-purple-500'], ['blue', 'bg-blue-500', 'ring-blue-500'], ['emerald', 'bg-emerald-500', 'ring-emerald-500'], ['rose', 'bg-rose-500', 'ring-rose-500']].map(([c, bg, ring]) => (
        <button key={c} onClick={() => setSettings({ ...settings, accentColor: c })} className={`w-10 h-10 rounded-full ${bg} hover:scale-110 transition-transform ${settings.accentColor === c ? `ring-4 ring-offset-4 ${ring} dark:ring-offset-zinc-900` : ''}`}/>
      ))}
    </div>
  </div>
</div>
...
{genFromSelection && (
<div className="mb-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 rounded-xl p-3 flex items-center gap-2">
<Sparkles size={14} className="text-amber-500 shrink-0"/>
<span className="text-xs font-bold text-amber-700 dark:text-amber-400 truncate">Using selected text ({genFromSelection.length} chars)</span>
<button onClick={() => setGenFromSelection(’’)} className=“ml-auto text-amber-500 hover:text-red-500”><X size={14}/></button>
</div>
)}
{!genFromSelection && (
<div className="flex items-center justify-between mb-6 gap-3">
<div className="w-full">
<label className="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-2 block">Start Pg</label>
<input type=“number” min={1} max={activeDoc.totalPages} value={startPage} onChange={e => setStartPage(Number(e.target.value))} className=“w-full bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-700 rounded-2xl px-4 py-3 text-center text-gray-800 dark:text-white font-mono font-bold outline-none focus:border-[var(–accent-color)] transition-all”/>
</div>
<div className="mt-5 text-gray-400 font-bold text-xs uppercase tracking-widest">TO</div>
<div className="w-full">
<label className="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-2 block">End Pg</label>
<input type=“number” min={1} max={activeDoc.totalPages} value={endPage} onChange={e => setEndPage(Number(e.target.value))} className=“w-full bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-700 rounded-2xl px-4 py-3 text-center text-gray-800 dark:text-white font-mono font-bold outline-none focus:border-[var(–accent-color)] transition-all”/>
</div>
</div>
)}
...
<span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{status.msg}</span>
<span className="text-[10px] font-black text-[var(--accent-color)]">{progress.done}/{progress.total}</span>
</div>
<div className="w-full bg-gray-200 dark:bg-zinc-800 rounded-full h-1.5">
...
<span className="text-xs font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 flex items-center gap-2"><CheckCircle2 size={14}/> Output Ready ({generated.data?.length || 1} items)</span>
<div className="flex gap-2">
...
<div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${m.role === 'user' ? 'bg-[var(--accent-color)]' : 'bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700'}`}>
{m.role === ‘user’ ? <UserCircle2 size={13} className="text-white"/> : <BrainCircuit size={13} className="text-[var(--accent-color)]"/>}
</div>
...
{loading && (
<div className="flex gap-3">
<div className="w-8 h-8 rounded-xl bg-white dark:bg-zinc-800 border flex items-center justify-center"><Loader2 size={13} className="text-[var(--accent-color)] animate-spin"/></div>
<div className="p-4 bg-white dark:bg-zinc-900 border rounded-2xl rounded-tl-sm flex gap-1.5 items-center">
{[0, 1, 2].map(i => <span key={i} className=“w-2 h-2 bg-gray-300 rounded-full animate-bounce” style={{ animationDelay: `${i * 0.15}s` }}/>)}
</div>
</div>
)}
...
<textarea ref={textareaRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === ‘Enter’ && !e.shiftKey) { e.preventDefault(); handleChat(); } }} placeholder={`Ask about ${contextMode === 'page' ? 'page ' + currentPage : 'the document'}...`} disabled={loading} className=“w-full bg-transparent p-3 pr-12 text-xs text-gray-800 dark:text-white outline-none resize-none max-h-32 custom-scrollbar” style={{ minHeight: ‘44px’ }}/>
...
{/* Tabs */}
<div className="flex gap-1 p-2 bg-gray-50 dark:bg-zinc-950 border-b border-gray-200 dark:border-zinc-800 shrink-0">
{[[‘exams’, ‘Exams’, docExams.length], [‘flashcards’, ‘Cards’, docCardSets.length], [‘notes’, ‘Notes’, docNotes.length]].map(([tab, label, count]) => (
<button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1 ${activeTab === tab ? 'bg-white dark:bg-zinc-800 text-[var(--accent-color)] shadow-sm border border-gray-200 dark:border-zinc-700' : 'text-gray-500 hover:text-gray-800 dark:hover:text-zinc-300'}`}>
{label} <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-black ${activeTab === tab ? 'bg-[var(--accent-color)] text-white' : 'bg-gray-200 dark:bg-zinc-700 text-gray-600 dark:text-zinc-400'}`}>{count}</span>
</button>
))}
</div>

```
  <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 pb-[100px] md:pb-6 space-y-3">
    {activeTab === 'exams' && (
      docExams.length === 0 ? <EmptyState icon={GraduationCap} text="No exams yet. Generate one from the AI Studio tab."/> :
      docExams.map(e => (
        <div key={e.id} className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 p-5 rounded-2xl group shadow-sm">
          <div className="flex justify-between items-start mb-3">
            <div className="flex-1">
              <p className="text-sm font-bold text-gray-800 dark:text-white mb-1">{e.title}</p>
              <div className="flex gap-2 flex-wrap">
                <span className="text-[9px] font-black text-emerald-600 bg-emerald-500/10 px-2 py-1 rounded-lg border border-emerald-500/20">{e.questions.length} Qs</span>
                {e.lastScore !== undefined && <span className={`text-[9px] font-black px-2 py-1 rounded-lg border ${e.lastScore >= 70 ? 'text-emerald-600 bg-emerald-500/10 border-emerald-500/20' : 'text-red-600 bg-red-500/10 border-red-500/20'}`}>Last: {e.lastScore}%</span>}
              </div>
            </div>
            <button onClick={() => setExams(exams.filter(ex => ex.id !== e.id))} className="p-2 text-gray-400 hover:text-red-500 rounded-lg"><Trash size={13}/></button>
          </div>
          <button onClick={() => setActiveItem({ type: 'exam', data: e })} className="w-full py-2.5 bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-600 dark:text-emerald-400 rounded-xl text-[10px] font-bold uppercase tracking-widest border border-emerald-500/20 flex items-center justify-center gap-2"><Play size={11} fill="currentColor"/> Take Exam</button>
        </div>
      ))
    )}
    {activeTab === 'flashcards' && (
      ...
      docCardSets.map(set => {
        const due = (set.cards || []).filter(c => c.nextReview <= Date.now()).length;
        return (
          <div key={set.id} className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm">
            <div className="flex justify-between items-start mb-3">
              <div>
                <p className="text-sm font-bold text-gray-800 dark:text-white mb-1">{set.title}</p>
                <div className="flex gap-2">
                  <span className="text-[9px] font-black text-[var(--accent-color)] bg-[var(--accent-color)]/10 px-2 py-1 rounded-lg border border-[var(--accent-color)]/20">{set.cards?.length || 0} Cards</span>
                  {due > 0 && <span className="text-[9px] font-black text-red-500 bg-red-500/10 px-2 py-1 rounded-lg border border-red-500/20 animate-pulse">{due} due</span>}
                </div>
              </div>
              <button onClick={() => setFlashcards(flashcards.filter(f => f.id !== set.id))} className="p-2 text-gray-400 hover:text-red-500 rounded-lg"><Trash size={13}/></button>
            </div>
    ...
    {activeTab === 'notes' && (
      ...
      docNotes.map(n => (
        <div key={n.id} onClick={() => setActiveItem({ type: 'note', data: n })} className="bg-white/80 dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-800 p-5 rounded-2xl relative group shadow-sm cursor-pointer hover:border-blue-500/30 transition-all">
...
<Icon size={48} className="text-gray-300 dark:text-zinc-700 mb-4"/>
<p className="text-xs text-gray-500 dark:text-zinc-500 leading-relaxed max-w-48">{text}</p>
</div>
);
}
...
const finalScore = Math.round((score / exam.questions.length) * 100);
return (
<div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-gray-50 dark:bg-[#0a0a0c]">
<div className={`w-32 h-32 rounded-full flex items-center justify-center mb-6 ${finalScore >= 70 ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
<span className={`text-4xl font-black ${finalScore >= 70 ? 'text-emerald-500' : 'text-red-500'}`}>{finalScore}%</span>
</div>
<h2 className="text-2xl font-black text-gray-800 dark:text-white mb-2">Exam Complete!</h2>
<p className="text-gray-500 dark:text-zinc-400 mb-2">{score}/{exam.questions.length} correct</p>
<p className={`text-sm font-bold mb-8 ${finalScore >= 70 ? 'text-emerald-600' : 'text-red-500'}`}>{finalScore >= 90 ? ‘Outstanding!’ : finalScore >= 70 ? ‘Good job!’ : ‘Keep studying!’}</p>
<div className="flex gap-3">
<button onClick={onBack} className="px-6 py-3 bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300 rounded-xl font-bold text-sm">Back to Vault</button>
<button onClick={() => { setCurrentQIndex(0); setScore(0); setAnswers([]); setSelectedAnswer(null); setShowFeedback(false); setExamComplete(false); }} className=“px-6 py-3 bg-[var(–accent-color)] text-white rounded-xl font-bold text-sm”>Retry</button>
</div>
</div>
);
}
...
<div className="w-full">
<h3 className="text-xs font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-4 flex items-center gap-2"><FlaskConical size={14}/> Laboratory Results</h3>
...
<div className="flex items-center gap-3">
<div className="flex-1 w-24 bg-gray-200 dark:bg-zinc-800 rounded-full h-1.5">
<div className=“bg-emerald-500 h-1.5 rounded-full transition-all” style={{ width: `${progressPct}%` }}/>
</div>
<span className="text-[9px] text-emerald-600 dark:text-emerald-500 font-black uppercase tracking-widest bg-emerald-500/10 px-2 py-1 rounded-lg">{currentQIndex + 1}/{exam.questions.length}</span>
<span className="text-[9px] text-gray-600 dark:text-zinc-400 font-black">Score: {score}</span>
</div>
</div>

```
  <div className="flex-1 overflow-hidden flex flex-col lg:flex-row relative">
    <div className={`flex-1 overflow-y-auto custom-scrollbar p-5 pb-[80px] ${hasLabs ? 'lg:w-[55%] lg:flex-none' : ''}`}>
      {q.vignette && (
      ...
      <div className="flex justify-between items-center">
        <button onClick={() => { if (currentQIndex > 0) { setCurrentQIndex(currentQIndex - 1); setSelectedAnswer(null); setShowFeedback(false); } }} disabled={currentQIndex === 0} className="px-4 py-2 text-gray-500 rounded-xl disabled:opacity-30 text-xs font-bold uppercase tracking-widest flex items-center gap-1"><ChevronLeft size={14}/> Previous</button>
        <button onClick={nextQuestion} disabled={!showFeedback} className="px-6 py-3 bg-gray-800 dark:bg-white text-white dark:text-zinc-950 disabled:bg-gray-200 dark:disabled:bg-zinc-800 disabled:text-gray-400 dark:disabled:text-zinc-600 rounded-xl text-xs font-black shadow-lg uppercase tracking-widest flex items-center gap-2">
          {currentQIndex < exam.questions.length - 1 ? 'Next' : 'Finish'} <ChevronRight size={14}/>
        </button>
      </div>
    </div>

    {hasLabs && (
      <div className="hidden lg:block w-[45%] border-l border-gray-200 dark:border-zinc-800 bg-gray-100/50 dark:bg-[#121214] overflow-y-auto custom-scrollbar p-6 pb-[80px] shadow-inner">
        {labSection}
      </div>
    )}
  </div>
...
<h2 className="text-xl font-black text-gray-800 dark:text-white mb-2">All Cards Reviewed!</h2>
<div className="grid grid-cols-4 gap-2 mb-6 mt-4">
{Object.entries(sessionStats).map(([k, v]) => (
<div key={k} className="text-center">
<p className="text-lg font-black text-gray-800 dark:text-white">{v}</p>
<p className="text-[9px] font-bold uppercase tracking-widest text-gray-500 capitalize">{k}</p>
</div>
))}
</div>
<button onClick={onBack} className="px-6 py-3 bg-[var(--accent-color)] text-white rounded-xl font-bold text-sm">Back to Vault</button>
</div>
);
...
<div className="flex items-center gap-3">
<div className="w-24 bg-gray-200 dark:bg-zinc-800 rounded-full h-1.5">
<div className=“bg-[var(–accent-color)] h-1.5 rounded-full transition-all” style={{ width: `${progressPct}%` }}/>
</div>
<span className="text-[9px] text-[var(--accent-color)] font-black uppercase tracking-widest bg-[var(--accent-color)]/10 px-2 py-1 rounded-lg">{currentIndex + 1}/{initialCards?.length || cards.length}</span>
</div>
</div>
<div className="flex-1 overflow-y-auto flex flex-col items-center justify-center p-4 pb-[100px]">
<div onClick={() => !isFlipped && setIsFlipped(true)} className=“w-full max-w-sm h-64 perspective-1000 cursor-pointer mb-8”>
...
<span className="absolute top-4 left-4 text-[9px] font-black uppercase tracking-widest text-gray-400 bg-gray-100 dark:bg-zinc-800 px-2 py-1 rounded-lg">Question</span>
<h2 className="text-lg font-bold text-gray-800 dark:text-white leading-relaxed">{currentCard.q}</h2>
<div className="absolute bottom-4 text-[9px] font-bold uppercase tracking-widest text-[var(--accent-color)]/60 animate-pulse">Tap to reveal</div>
</div>
...
<span className="absolute top-4 left-4 text-[9px] font-black uppercase tracking-widest text-white/70 bg-white/10 px-2 py-1 rounded-lg">Answer</span>
<p className="text-base font-bold text-white leading-relaxed">{currentCard.a}</p>
</div>
</div>
</div>
<div className={`w-full max-w-sm grid grid-cols-2 gap-3 transition-opacity ${isFlipped ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
{[
{ label: ‘Again’, q: 0, cls: ‘text-red-600 dark:text-red-400 bg-red-500/10 border-red-500/20 hover:bg-red-500 hover:text-white hover:border-red-500’ },
{ label: ‘Hard’, q: 3, cls: ‘text-yellow-600 dark:text-yellow-400 bg-yellow-500/10 border-yellow-500/20 hover:bg-yellow-500 hover:text-white hover:border-yellow-500’ },
{ label: ‘Good’, q: 4, cls: ‘text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20 hover:bg-emerald-500 hover:text-white hover:border-emerald-500’ },
{ label: ‘Easy’, q: 5, cls: ‘text-blue-600 dark:text-blue-400 bg-blue-500/10 border-blue-500/20 hover:bg-blue-500 hover:text-white hover:border-blue-500’ },
].map(({ label, q, cls }) => (
<button key={label} onClick={() => handleRate(q)} className={`py-3 rounded-2xl text-xs font-black uppercase tracking-widest border transition-all ${cls}`}>{label}</button>
))}
</div>
</div>
...
<button onClick={copyToClipboard} className="text-blue-600 dark:text-blue-400 p-2 rounded-xl hover:bg-blue-500/10 transition-colors"><Clipboard size={16}/></button>
</div>
...
<div className="text-sm text-gray-700 dark:text-zinc-300 whitespace-pre-wrap leading-relaxed bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm">{note.content}</div>
</div>
</div>
);
}