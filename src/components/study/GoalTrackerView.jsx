...
export default function GoalTrackerView({ flashcards, exams, addToast }) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar scroll-content p-4 space-y-4">
      <div className="flex items-center justify-between"><h1 className="text-2xl font-black">Goal Tracker</h1><button onClick={() => { setDraft(goals); setEditing(!editing); }} className="glass px-4 py-2 rounded-xl text-sm font-black" style={{ border: '1px solid var(--border)', color: 'var(--accent)' }}>{editing ? 'Cancel' : ' Edit'}</button></div>
