...
const URO_SECTIONS = [
  { id: 'bph', title: 'BPH & Prostate', icon: '',
    items: [
      { term: 'Benign Prostatic Hyperplasia', def: 'Prevalence increases with age (50% at 50, 90% at 80). LUTS: frequency, urgency, nocturia, weak stream, hesitancy, incomplete emptying, post-void dribbling. IPSS (International Prostate Symptom Score) for severity.', detail: 'Diagnosis: DRE (smooth, enlarged, firm, non-tender), UA (rule out UTI), PSA (discuss), PVR (post-void residual  200-300 mL is significant). Uroflowmetry, pressure-flow studies for equivocal cases.', pearl: 'Treatment: Mild (IPSS <8): watchful waiting, lifestyle (reduce fluids before bed, limit caffeine/alcohol). Moderate-severe: Alpha-blockers (tamsulosin, alfuzosin)  rapid symptom relief (days to weeks). 5-reductase inhibitors (finasteride, dutasteride)  reduce prostate volume over months, prevent progression. Combination therapy (MTOPS, CombAT trials). PDE5 inhibitor (tadalafil 5 mg daily  for BPH + ED). Surgical: TURP (gold standard), HoLEP (laser enucleation  comparable, less bleeding), UroLift (prostatic urethral lift  preserves ejaculatory function), water vapor therapy (Rezm), aquablation (robotic waterjet).' },
