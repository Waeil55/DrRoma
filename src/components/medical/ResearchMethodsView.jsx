...
const RESEARCH_SECTIONS = [
  ...
  { id: 'biostats', title: 'Biostatistics', icon: '',
    items: [
      ...
      { term: 'Type I and Type II Errors', def: 'Type I (): reject null when null is TRUE = false positive ("convict an innocent"). Typically  = 0.05. Type II (): fail to reject null when null is FALSE = false negative ("acquit a guilty"). Power = 1-.', detail: 'Power: probability of detecting a true effect if it exists. Standard target: 80% (  0.20). Factors increasing power: larger sample size, larger effect size, higher alpha, lower variability.', pearl: 'Type I error is controlled by  level (0.05). Type II error is controlled by power analysis (sample size calculation BEFORE the study). A "negative" study (P >0.05) may be underpowered  check the power. Bonferroni correction: when doing multiple comparisons, adjust  (divide 0.05 by number of comparisons) to control overall Type I error.' },
